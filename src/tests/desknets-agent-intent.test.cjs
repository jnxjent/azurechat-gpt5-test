...
test("routes a candidate choice when only the numbered assistant reply remains in history", () => {
  ...
  assert.equal(shouldRouteToDeskNetsAgent("6", candidates), true);
  assert.equal(shouldRouteToDeskNetsAgent("6", [{ role: "tool", name: "desknets_schedule_agent", content: "6. 10/6 10:00〜11:00" }]), true);
  assert.equal(shouldRouteToDeskNetsAgent("では６で", [{ role: "assistant", content: "1. 売上\n2. 利益" }]), false);
  assert.equal(shouldRouteToDeskNetsAgent("6", [{ role: "assistant", content: "1. 売上\n2. 利益" }]), false);
