      ...
      messages: [
        {
          ...
          content:
            ...
            [
              ...
              "- Use `create_pptx` when the user wants a brand-new PowerPoint from scratch, OR when a slide structure was already discussed in this conversation and the user wants to ENRICH it using an uploaded PDF as reference material ('PDFを参考に内容を厚くして' / '追記して' / '拡充して'). In the enrichment case, base the slides parameter on the previously discussed slide structure and fill in the bullets with content from the document context or sl_doc_search results.",
              "- Use `convert_doc_to_pptx` only when the user wants the PDF/image itself converted to PPT ('PPTに変換して' / 'スライド化して'), and a `file_url:` or `fileUrl:` line exists in the context. Do NOT use it when a slide structure was already discussed and the PDF is only a reference — use `create_pptx` instead.",
              ...
              "- Do NOT use `edit_word`, `edit_pptx`, `convert_sp_to_pptx`, or any other tool for PDF→Word conversion.",
