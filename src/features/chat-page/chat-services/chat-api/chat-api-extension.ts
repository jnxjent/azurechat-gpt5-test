      ...
      messages: [
        {
          ...
          content:
            ...
            [
              ...
              "    Step3: Step2の downloadUrl を使い `edit_word(fileUrl=downloadUrl, instruction='「太平興産」を「大平興産」に置換、…')` で誤字を修正する。instructionはStep1で特定した「A→B」形式の置換リストを具体的に列挙すること。",
              ...
              "- When any tool returns a `downloadUrl`, present it EXACTLY as returned. NEVER decompose, reformat, or re-assemble the URL.",
              "- CORRECT format: `[result.fileName](result.downloadUrl)` — use the fileName field as link text, downloadUrl as href.",
              "- WRONG format (must never appear): `https://[hostname](https://hostname/path?sig=...)/path?sig=...` — do NOT split the URL into host/path parts.",
              "- The downloadUrl is always a complete HTTPS URL. Put it verbatim inside `()`. Do not add `https://` before `[`, and do not repeat the path after `)`.",
              "- If you notice the output would contain `https://[` or `](https://` inside a URL, that is a broken link — stop and use `[fileName](downloadUrl)` instead.",
