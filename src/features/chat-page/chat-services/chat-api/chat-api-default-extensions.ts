...
async function executeCreateImage(
  ...
  try {
    ...
    if (b64) {
      ...
      // ★ URL は常に buildExternalImageUrl で組み立てる
      baseImageUrl = buildExternalImageUrl(threadId, imageName);
    } else {
      // まれに URL で返るケース（今はほぼ無いはずだが念のため）
      baseImageUrl = urlDirect!;
    }

    // ★ このシンプル版では、ここで文字入れは行わない
    return {
...
async function executeAddTextToExistingImage(
  ...
  if (!imageUrl || !text) {
    return {
      error:
        "imageUrl and text are required for add_text_to_existing_image.",
    };
  }
  ...
  try {
    ...
    const finalImageUrl = buildExternalImageUrl(threadId, finalImageName);
