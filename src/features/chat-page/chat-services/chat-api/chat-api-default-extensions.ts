...
async function planCompanyProfileSlides(
  ...
  try {
    ...
    const systemPrompt = `You are an expert PowerPoint presentation designer. Design 7-8 company profile slides in Japanese. You are the DECISION MAKER for visual design — layout choice, information hierarchy, and text treatment are YOUR responsibility. The rendering engine will faithfully execute your decisions and only enforce safety constraints (no overflow, no overlap).

## Your Design Responsibilities

For each slide, decide:
1. What is the single most important thing to communicate?
2. Which layout best fits the information density and type?
3. How to present text concisely without losing meaning
4. Which information should be primary (large/prominent) vs supporting (small/note)
5. How to use color variety — avoid repeating the same color role

## Available layoutTypes

- "bullets": Bullet list. Use for: key messages, features, rationale. Fields: title, bullets (max 4, 1-2 sentences each)
- "company-overview": Company profile with lead text + 4 metric cards + optional callout. Fields: title, bullets[], leadText (2-4 sentences), metrics (max 4), callout?
- "metric-cards": Full-width KPI cards — use when numbers/facts ARE the message. Fields: title, bullets[], metrics (max 4)
- "process-cards": Step-by-step flow. Fields: title, bullets[], subtitle (1 sentence), steps (2-4), benefits (2-4)
- "timeline": Horizontal timeline. Fields: title, bullets[], subtitle?, steps (3-5), benefits?
- "multi-column": Side-by-side comparison. Fields: title, bullets[], columns (2-3: {header, bullets[]})
- "table": Structured table. Fields: title, bullets[], tableRows (first row = header)
- "closing": Call to action. Fields: title, bullets (3-4 action items)

## Metric Card Design Rules (CRITICAL)

Each metric in metrics[] must have:
- **value**: SHORT display value — max 15 characters. Examples:
  - Address → "静岡県浜松市" (city only, NOT full address)
  - Stock market → "東証プライム" (primary market only, NOT "東証プライム・名証プレミア市場上場")
  - Date → "1952年" (year only, NOT "1952年4月1日")
  - Headcount → "457名"
- **note**: Supplementary detail. Examples:
  - "〒431-3122 中央区有玉南町2163番地"
  - "名証プレミア市場にも上場"
  - "2025年6月末時点・連結"
- **iconKey**: Choose from: calendar / location / stock / network / people / chart / building / gear / verified / star / truck / gear / archive / shield / coins / leaf / eye / lightbulb / rocket
- **colorRole**: "primary" (teal-green, for key facts), "accent" (copper, for contrast), "neutral" (dark green, for supporting). Alternate across cards to avoid monotony.

## Step Icon Keys

For process-cards / timeline steps, assign iconKey from:
- Waste/logistics: truck / gear / archive / shield / coins / leaf / eye
- General business: building / people / chart / star / verified / lightbulb / rocket / network

## Design Quality Rules

1. Cover slide is auto-generated — do NOT include a "表紙" slide
2. NEVER use facts not found in the provided facts object — omit rather than invent
3. Vary layoutType — never repeat consecutively
4. bullets: max 4 items; metrics: max 4; steps: max 4 (with non-empty body)
5. Total: 7-8 slides
6. Use colorRole alternation in metrics: primary / accent / primary / accent gives natural contrast
7. density field: "low" = few key items displayed large, "high" = comprehensive detail
8. textTreatment: "short" = keywords only, "normal" = 1-2 sentences, "explanatory" = fuller description

Return ONLY this JSON (no markdown, no explanation):
{"slides":[{"title":"...","bullets":[],"layoutType":"company-overview","visualIntent":"...","density":"medium","textTreatment":"normal","leadText":"...","metrics":[{"label":"創業","value":"1952年","note":"1952年4月創業","iconKey":"calendar","colorRole":"primary"},...]},...]}`;
      ...
      messages: [
        {
          role: "system",
          content:
            "You are a presentation specialist. Given a slide skeleton and reference document content, " +
            "rewrite the slide bullets with specific facts, numbers, and details found in the document. " +
...
- company-overview 時は leadText/metrics/callout フィールドも必ず設定
- process-cards 時は steps/benefits フィールドも必ず設定
...
チェック項目:
1. 【1スライド1メッセージ】タイトルは「主要機能」等のトピック名ではなく「話せば議事録が自動で仕上がります」のようにベネフィットで言い切る形にする。英字の小見出し(eyebrow)も bullets[0] 等の先頭に "EXECUTIVE SUMMARY | " 等の形で付記してよい。
2. 【短文化】1つの bullet に複数の論点を詰め込まない。1項目＝1メッセージで30字以内を目標。冗長な係り受けを削る。
3. 【KPI抜き出し】数値（工数削減・利用者数・コスト等）は文章に埋めず callout や metrics に独立して出す。
4. colorRole が意味ベースか（数値・実績・差別化 → accent、基本情報 → primary、補足 → neutral）
5. layoutType が内容に合っているか
6. metrics/steps/bullets の情報量が多すぎないか（各最大4項目）
7. 【文体統一】bullets・leadText・callout・steps の本文はすべて「です/ます調」に統一すること。体言止め・言い切り（〜する、〜推進、〜実施）は「〜しています」「〜できます」等に書き直す。
...
{"slides":[...]} の形式でJSONのみ返してください。`;
    ...
    return refined.map((s, i) => ({
      ...
      ...slides[i],
      ...s,
      bullets: Array.isArray(s.bullets) ? s.bullets : (slides[i]?.bullets ?? []),
...
async function executeCreatePptx(
  ...
  if (spDocQuery) {
    ...
    // 会社紹介モード: Web事実収集 → LLM抽出 → LLMスライド設計
    const companyName = extractCompanyNameFromTitle(title);
    // 社名を必ず含む検索クエリを構築（searchQueryに社名がない場合は上書き）
    ...
    const facts = await extractCompanyFacts(companyName, evidence);
    console.log(`[create_pptx] facts extracted: ${JSON.stringify(facts).slice(0, 300)}`);
    const planned = await planCompanyProfileSlides(
      title, facts, userMessage ?? "", designInstruction
    );
    if (planned.length > 0) {
      finalSlides = planned;
    } else {
